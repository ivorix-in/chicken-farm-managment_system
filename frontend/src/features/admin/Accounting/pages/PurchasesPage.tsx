...
import { Plus, ArrowDownRight, ShoppingCart, Edit2, Trash2 } from 'lucide-react';
...
const PURCHASE_CATEGORIES = ['FEED_PURCHASE', 'CHICK_PURCHASE', 'MEDICINE_PURCHASE', 'OTHER'];
...
export default function PurchasesPage() {
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      toast.success('Purchase record has been deleted successfully.');
      queryClient.invalidateQueries({ queryKey: ['transactions'] });
      queryClient.invalidateQueries({ queryKey: ['pnlSummary'] });
    },
    onError: (err: any) => {
      const msg = err.response?.data?.message || 'Failed to delete purchase record';
      toast.error(msg);
    },
  });
  ...
  const handleDelete = (id: string) => {
    Swal.fire({
      title: 'Are you sure?',
      text: 'Do you want to delete this purchase transaction? This action cannot be undone.',
      icon: 'warning',
  ...
  // Filter transactions to only purchases (Expenses within purchase categories)
  const purchaseTransactions = transactions.filter(
    (tx) => tx.type === 'EXPENSE' && PURCHASE_CATEGORIES.includes(tx.category)
  );
    ...
    <div className="p-8 max-w-7xl mx-auto space-y-8 text-left">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
            <ShoppingCart className="text-[#00A859]" size={28} />
            Purchase Management
          </h1>
          <p className="text-sm text-gray-500 mt-1">
            Track feed, chick, and medicine purchases for integration farms.
          </p>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50/50 border-b border-gray-100 text-[11px] uppercase tracking-wider text-gray-500 font-semibold">
                ...
                <th className="p-4">Purchase Type</th>
              ...
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={5} className="p-8 text-center text-gray-400">
                    No purchase transactions recorded yet. Click 'Record Purchase' to add one.
                  </td>
                </tr>
              ) : (
                purchaseTransactions.map((tx: Transaction) => (
      ...
      {isModalOpen && (
        <TransactionCreateModal
          ...
          defaultType="EXPENSE"
          allowedCategories={PURCHASE_CATEGORIES}
        />
      )}
    </div>
  );
